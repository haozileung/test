...
public class JdbcDaoDbUtilsImpl implements JdbcDao {
	...
	public int delete(Class<?> clazz, Long id) {
		...
		try {
			return getRunner().update(boundSql.getSql(), boundSql.getParameters().toArray());
	...
	public <T> T get(Class<T> clazz, Long id) {
		...
		List<T> list = null;
		try {
			list = getRunner().query(boundSql.getSql(), new BeanListHandler<T>(clazz),
	...
	public <T> T get(Criteria criteria, Long id) {
		...
		List<T> list = null;
		try {
			list = (List<T>) getRunner().query(boundSql.getSql(),
	...
	protected String getDialect() {
		if (StringUtils.isBlank(dialect)) {
			try {
				dialect = this.runner.getDataSource().getConnection().getMetaData().getDatabaseProductName()
						.toUpperCase();
			} catch (SQLException e) {
				return null;
			}
	...
	protected Long insert(final BoundSql boundSql) {
		try {
			return this.runner.insert(boundSql.getSql(), new ScalarHandler<Long>(), boundSql.getParameters().toArray());
		} catch (SQLException e) {
			logger.error(e.getMessage(), e);
			return 0L;
		}
	...
	public int queryCount(Criteria criteria) {
		...
		try {
			return getRunner().query(boundSql.getSql(), new ScalarHandler<Integer>(),
					boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public List<Map<String, Object>> queryForList(Criteria criteria) {
		...
		try {
			mapList = getRunner().query(boundSql.getSql(), new MapListHandler(), boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public <T> List<T> queryList(Class<?> clazz) {
		...
		List<?> list = null;
		try {
			list = getRunner().query(boundSql.getSql(), new BeanListHandler<T>((Class<T>) clazz),
					boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public <T> List<T> queryList(Criteria criteria) {
		...
		List<T> list = null;
		try {
			list = getRunner().query(boundSql.getSql(), new BeanListHandler<T>((Class<T>) criteria.getEntityClass()),
					boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public <T> List<T> queryList(T entity) {
		...
		List<T> list = null;
		try {
			list = getRunner().query(boundSql.getSql(), new BeanListHandler<T>((Class<T>) entity.getClass()),
					boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public <T> T querySingleResult(T entity) {
		...
		List<T> list = null;
		try {
			list = (List<T>) getRunner().query(boundSql.getSql(), new BeanListHandler<T>((Class<T>) entity.getClass()),
					boundSql.getParameters().toArray());
		} catch (SQLException e) {
	...
	public void save(Criteria criteria) {
		final BoundSql boundSql = criteria.build(true, getNameHandler());
		try {
			getRunner().update(boundSql.getSql(), boundSql.getParameters().toArray());
		} catch (SQLException e) {
